...
Another major challenge came when implementing the pipelining modules and wiring them correctly in top.sv. At first, I struggled to identify which signals needed to pass through each pipeline register and how the control logic should flow between stages. This made integrating the registers, hazard unit, and forwarding logic difficult and error-prone. After revisiting the lecture slides and diagrams to rebuild a clearer understanding of the pipelined datapath, the structure finally clicked, and connecting the modules in the top-level design became much more straightforward.
