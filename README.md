...
One of the main challenges we faced as a team was communication, particularly when working collaboratively on GitHub. At the start of the project, we underestimated how important regular coordination would be, which sometimes led to duplicated work or changes unintentionally overwriting someone else’s progress. Over time, we improved by discussing design decisions more openly, giving regular updates, and managing branches more carefully, which made a noticeable difference to our workflow.

Another challenge was the way we structured the development process. In hindsight, we should have focused on completing and verifying the single cycle processor before moving on to the pipelined design. Working on both in parallel meant that changes to core behaviour in the single-cycle CPU sometimes caused unexpected issues in the pipeline, leading to extra rework and more complex debugging. A more staged approach would have been more efficient.

Finally, we initially misunderstood the cache requirements and assumed the cache needed to be write-back. This led us to overcomplicate the design and spend time dealing with timing and control issues that were not actually required. Once we clarified the specification, we were able to simplify the design and complete a correct, verifiable cache implementation. This experience reinforced the importance of carefully interpreting project requirements before committing to a particular design approach.

Despite these challenges, the final design is stable, fully verified, and reflects a much stronger understanding of system level CPU design and team based hardware development.
